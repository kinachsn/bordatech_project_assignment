...
// Timer 2 interrupt handler (Sampling)
void TIM2_IRQHandler() {
    if (TIM2->SR & 0x01) {
        ...
        int len = snprintf(data_buffer, sizeof(data_buffer), "Temp: %.2f C, Light: %.2f lux, Pressure: %.2f hPa\r\n",
...
void TIM5_IRQHandler() {
    if (TIM5->SR & 0x01) {  // If update interrupt occurs
        ...
        // Calculate statistics for each sensor data
        float min_temp = calculate_min(temp_buffer, WINDOW_SIZE);
        float max_temp = calculate_max(temp_buffer, WINDOW_SIZE);
        float std_dev_temp = calculate_std_dev(temp_buffer, WINDOW_SIZE);

        float min_lux = calculate_min(light_buffer, WINDOW_SIZE);
        float max_lux = calculate_max(light_buffer, WINDOW_SIZE);
        float std_dev_lux = calculate_std_dev(light_buffer, WINDOW_SIZE);

        float min_pascal = calculate_min(press_buffer, WINDOW_SIZE);
        float max_pascal = calculate_max(press_buffer, WINDOW_SIZE);
        float std_dev_pascal = calculate_std_dev(press_buffer, WINDOW_SIZE);

        char data_buffer[256];
                int len = snprintf(data_buffer, sizeof(data_buffer),
                                   "Temp: %.2f C (Min: %.2f, Max: %.2f, Std Dev: %.2f), "
                                   "Light: %.2f lux (Min: %.2f, Max: %.2f, Std Dev: %.2f), "
                                   "Pressure: %.2f hPa (Min: %.2f, Max: %.2f, Std Dev: %.2f)\r\n",
                                   filtered_temp, min_temp, max_temp, std_dev_temp,
                                   filtered_lux, min_lux, max_lux, std_dev_lux,
                                   filtered_pascal, min_pascal, max_pascal, std_dev_pascal);
